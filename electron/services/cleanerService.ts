...
export function getCategoryConfigs(): CategoryConfig[] {
  ...
  return [
    ...
    },
    {
      ...
      description: 'Caches from npm, pnpm, yarn, pip, cargo, and gradle that accumulate gigabytes of build artifacts.',
      icon: 'Code2',
      safeToClean: true,
      getPaths: () => {
        ...
        return [
          ...
          path.win32.join(localAppData, 'Yarn', 'Cache'),
          path.win32.join(localAppData, 'pip', 'cache'),
          path.win32.join(userProfile, '.gradle', 'caches'),
          path.win32.join(userProfile, '.cargo', 'registry', 'cache'),
        ]
      },
    },
  ]
 ...
 */
export async function inspectRecycleBin(): Promise<{ sizeBytes: number; fileCount: number }> {
  try {
    const psScript = `
 ...
 */
export async function clearRecycleBinNative(): Promise<{ success: boolean; bytesReclaimed: number }> {
  try {
 ...
 */
export async function cleanDirectoryContents(dirPath: string): Promise<{
  cleanedBytes: number
  deletedCount: number
  skippedCount: number
