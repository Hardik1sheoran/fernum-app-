...
import type { SearchQueryOptions, SearchResultItem, SearchResultResponse, FileCategory } from '../../shared/types'
...
import { loadScanCache } from './scanCache'
...
export async function searchDiskFiles(
  ...
  while (queueHead < queue.length && searchedDirs < MAX_DIRS && results.length < limit * 2) {
    ...
    const candidateFiles: { fullPath: string; name: string; ext: string; category: FileCategory }[] = []
